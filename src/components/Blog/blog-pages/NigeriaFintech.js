...
const NigeriaFintech = ({
    ...
    const data = blogs.items.filter(
        (e) => e.title === 'Nigeria’s fintech + Financial Inclusion'
    );
    const finalBlog = data[0];

    const shuffledData = blogs.items
        .filter((e) => e.title !== 'Nigeria’s fintech + Financial Inclusion')
